...
ARG KEYBALL_REPO_SSH="git@github.com:ShuShiotani71/keyball.git"
ARG KEYBALL_REPO_HTTPS="https://github.com/ShuShiotani71/keyball.git"
# keyball config
...
# for public repos better to clone with https
RUN git clone ${KEYBALL_REPO_HTTPS} keyball
# change origin to ssh for later; for pushing would still need to authenticate so common to use ssh
RUN cd keyball && git remote set-url origin ${KEYBALL_REPO_SSH}
